...
ERR HAL_SYSTICK_Init(SYSTICK_CFG *SYS)
{
	...
	if( SYS != NULL)
	{
		...
		switch( SYS ->CLCK_DIV )
		{
			...
			if(SYS->SystickTick == SYSTICK_MS)
				LOAD_VAL = SYS->LOAD_VAL*(SYSCTICK_CLK/1000); //1ms
			else if(SYS->SystickTick == SYSTICK_US)
				LOAD_VAL = SYS->LOAD_VAL*(SYSCTICK_CLK/1000000); //1us
			SYSTICK->LOAD = LOAD_VAL;
		#if RTOS_SELF == EN
			CLEAR_MSK(SYSTICK_PRI,0xFFFFFFFF);
